/*Modulación commonjs*/

//const user = require('../models/user');

/*Para manipular el modelo hay que importarlo
busca en index.js */
const Task = require('../models/').Task;
const User = require('../models/').User;

// module.exports = {
//     //Funcion que es comun para peticiones http
//     home: function(req, res) {
//         //promesa que se resuleve con el then
//         //funcion recibe los registros de la bd
//         Task.findAll().then(function(tasks) {
//             res.render('tasks/index', { //datos que se van a compartir
//                 tareas: tasks
//             });
//         });
//     }
// };

module.exports = {
    index: function(req, res) {
        Task.findAll().then((tasks) => {
            res.render('tasks/index', {
                tasks: req.user.tasks
            });
        })
    },
    show: function(req, res) {
        Task.findByPk(req.params.id, {
            include: [{
                    model: User,
                    as: 'user'
                },
                'categories'
            ]
        }).then(function(task) {
            res.render('tasks/show', { task })
        })
    },
    edit: function(req, res) {
        Task.findByPk(req.params.id).then(function(task) {

            res.render('tasks/edit', { task })
        });
    },
    delete: function(req, res) {
        Task.destroy({
            where: {
                id: req.params.id
            }
        }).then(function(contadorElementosEliminados) {
            res.redirect('/tasks')
        })
    },
    create: function(req, res) {
        Task.create({
            description: req.body.description,
            userId: req.user.id,
        }).then(result => {
            res.json(result);
        }).catch(err => {
            console.log(err);
            res.json(err);
        })
    }, //Criterio para saber que registro
    update: function(req, res) {
        // task.addCategories([1,5])
        let task = Task.findByPk(req.params.id).then(task => {
            task.description = req.body.description;
            task.save().then(() => {
                let categoryIds = req.body.categories.split(","); //"1,5,4" => [1,5,4]

                task.addCategories(categoryIds).then(() => {
                    res.redirect(`/tasks/${task.id}`);
                })
            })
        })
    },
    new: function(req, res) {
        res.render('tasks/new');

    }
};